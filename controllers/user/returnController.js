...
const initiateReturn = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        'An unexpected error occurred while processing your return request.',
    });
  }
};
  ...
  try {
    ...
    if (status === 'approved') {
      const refundCalculation =
        order.calculateReturnRefundAmount(returnedItems);
      ...
      if (
        ...
        try {
          ...
          const refundBreakdown = refundDetails
            .map(
              (detail) =>
          ...
          const allReturned = order.orderedItems.every((item) =>
            ['Return Approved', 'Cancelled'].includes(item.status),
          );
          if (allReturned) {
