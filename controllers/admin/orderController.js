...
const getOrderList = async (req, res) => {
  try {
    ...
    for (const order of orders) {
      const userAddressData = await Address.findOne({
        userId: order.user,
      }).lean();

      const specificAddress = userAddressData.address.find((addr) =>
        addr._id.equals(order.address),
      );
...
const acceptReturnRequest = async (req, res) => {
  try {
    ...
    const existingTransaction = wallet.transactions.find(
      (transaction) =>
        transaction.description === `Refund for returned order ${orderId}`,
    );
