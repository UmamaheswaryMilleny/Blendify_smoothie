  ...
  try {
    ...
    if (filter) {
      ...
      return res.status(400).json({
        error: 'Please provide either a filter or a valid date range',
      });
    }
    ...
    if (orders.length > 0) {
      ...
      const overallDeliveryCharge = orders.reduce(
...
const salesReportDownload = async (req, res) => {
  try {
    ...
    if (
      !orders ||
      overallSalesCount === undefined ||
      overallOrderAmount === undefined ||
      overallDiscount === undefined ||
      overallDeliveryCharge === undefined ||
